...
def run():
        ...
        def calculate(self):
            ...
            def function_multiply(first, second, multiplyer):
                ...
                if choosing_get.get() == first and display_info.get() == second:
                    ...
                    if str(rounded_result)[-2:] == '.0':
                        fixed_result = str(rounded_result)[:-2]
                        formated_result = '{:,}'.format(int(fixed_result))
                        txtDisplay['text'] = str(formated_result)
                    else:
            ...
            # Degree
            function_same('Degrees')
            function_multiply('Radians' , 'Degrees' , (180 / math.pi))
            function_multiply('Gradians', 'Degrees', (180 / 200))


            # Radians
            function_multiply('Degrees', 'Radians', (math.pi / 180))
            function_same('Radians')
            function_multiply('Gradians', 'Radians', (math.pi / 200))


            # Gradians
            function_multiply('Degrees', 'Gradians', (200 / 180))
            function_multiply('Radians', 'Gradians', (200 / math.pi))
            function_same('Gradians')


    Angle_calculations = Angle()
    ...
    for j in range(4, 7):
        for k in range(3):
            ...
            button[i]['command'] = lambda x=numbers[i]: Angle_calculations.entering_numbers(x)
            i += 1
    ...
    Zero_button = Button(root, text='0', width=9, height=1, font=('arial', 20, 'bold'), bd=4,
                         bg='white', activebackground='gray78' , command = lambda : Angle_calculations.entering_numbers(0))
    ...
    Dot_button = Button(root, text='.', width=9, height=1, font=('arial', 20, 'bold'), bd=4,
                         bg='gray91', activebackground='gray78' , command = lambda : Angle_calculations.entering_numbers('.'))
    ...
    Result_button = Button(root , text = 'Calculate' , font=('arial', 20, 'bold') ,  width=9, height=1 , bd=4 ,
                           bg="SteelBlue1" , activebackground = 'SteelBlue3' , command = Angle_calculations.calculate)
    ...
    Plus_minus_button = Button(root, text=chr(177), font=('arial', 20, 'bold'), width=9, height=1, bd=4,
                           bg="gray91", activebackground='gray78' , command = Angle_calculations.plusminus)
    ...
    Clear_last_button = Button(root, text='⌫', width=9, height=1, font=('arial', 20, 'bold'), bd=4,
                               bg="gray91", activebackground='gray78' , command = Angle_calculations.remove_last)
    ...
    AllClear_button = Button(root, text=chr(67), width=9, height=1, font=('arial', 20, 'bold'), bd=4,
                             bg="gray91", activebackground='gray78' , command = Angle_calculations.clear_entry)
