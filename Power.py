...
def run():
    ...
    class Power():
        ...
        def calculate(self):
            ...
            def watts_to_watts():
                if choosing_get.get() == 'Watts' and display_info.get() == 'Watts':
            ...
            def watts_to_kilowatts():
                if choosing_get.get() == 'Watts' and display_info.get() == 'Kilowatts':
                    result = float(txtDisplay_get.get()) / 1_000
                    if str(result)[-2:] == '.0':
                        fixed_result = str(result)[:-2]
                        formated_result = '{:,}'.format(int(fixed_result))
                        txtDisplay['text'] = str(formated_result)
            ...
            def kilowatts_to_watts():
                if choosing_get.get() == 'Kilowatts' and display_info.get() == 'Watts':
                    result = float(txtDisplay_get.get()) * 1_000
                    if str(result)[-2:] == '.0':
                        fixed_result = str(result)[:-2]
                        formated_result = '{:,}'.format(int(fixed_result))
            ...
            def kilowatts_to_kilowatts():
                if choosing_get.get() == 'Kilowatts' and display_info.get() == 'Kilowatts':
                    result = float(txtDisplay_get.get())
                    if str(result)[-2:] == '.0':
                        fixed_result = str(result)[:-2]
                        formated_result = '{:,}'.format(int(fixed_result))
            ...
            kilowatts_to_kilowatts()

    Power_calculations = Power()
    ...
    for j in range(4, 7):
        for k in range(3):
            ...
            button[i]['command'] = lambda x=numbers[i]: Power_calculations.entering_numbers(x)
            i += 1
    ...
    Zero_button = Button(root, text='0', width=9, height=1, font=('arial', 20, 'bold'), bd=4,
                         bg='white', activebackground='gray78' , command = lambda : Power_calculations.entering_numbers(0))
    ...
    Dot_button = Button(root, text='.', width=9, height=1, font=('arial', 20, 'bold'), bd=4,
                         bg='gray91', activebackground='gray78' , command = lambda : Power_calculations.entering_numbers('.'))
    ...
    Result_button = Button(root , text = 'Calculate' , font=('arial', 20, 'bold') ,  width=9, height=1 , bd=4 ,
                           bg="SteelBlue1" , activebackground = 'SteelBlue3' , command = Power_calculations.calculate)
    ...
    Plus_minus_button = Button(root, text=chr(177), font=('arial', 20, 'bold'), width=9, height=1, bd=4,
                           bg="gray91", activebackground='gray78' , command = Power_calculations.plusminus)
    ...
    Clear_last_button = Button(root, text='⌫', width=9, height=1, font=('arial', 20, 'bold'), bd=4,
                               bg="gray91", activebackground='gray78' , command = Power_calculations.remove_last)
    ...
    AllClear_button = Button(root, text=chr(67), width=9, height=1, font=('arial', 20, 'bold'), bd=4,
                             bg="gray91", activebackground='gray78' , command = Power_calculations.clear_entry)
