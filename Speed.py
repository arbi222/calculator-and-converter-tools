...
def run():
    ...
    class Speed():
        ...
        def calculate(self):
            ...
            # Meter per second
            function_same('M/s')
            function_divide('Km/h' , 'M/s' , 3.6)
            function_divide('Feet/second', 'M/s', 3.280839895)
            function_divide('Miles/hour', 'M/s', 2.2371364653)

            # Kilometer per hour
            function_multiply('M/s' , 'Km/h' , 3.6)
            function_same('Km/h')
            function_multiply('Feet/second', 'Km/h', 1.09728)
            function_multiply('Miles/hour', 'Km/h', 1.6092)

            # Feet per second
            function_multiply('M/s', 'Feet/second', 3.28084)
            function_divide('Km/h', 'Feet/second', 1.0972805)
            function_same('Feet/second')
            function_multiply('Miles/hour', 'Feet/second', 1.466535)

            # Miles per hour
            function_multiply('M/s', 'Miles/hour', 2.237136)
            function_divide('Km/h', 'Miles/hour', 1.6091994715)
            function_divide('Feet/second', 'Miles/hour', 1.4665358817)
            function_same('Miles/hour')


    Speed_calculations = Speed()
    ...
    for j in range(4, 7):
        for k in range(3):
            ...
            button[i]['command'] = lambda x=numbers[i]: Speed_calculations.entering_numbers(x)
            i += 1
    ...
    Zero_button = Button(root, text='0', width=9, height=1, font=('arial', 20, 'bold'), bd=4,
                         bg='white', activebackground='gray78' , command = lambda : Speed_calculations.entering_numbers(0))
    ...
    Dot_button = Button(root, text='.', width=9, height=1, font=('arial', 20, 'bold'), bd=4,
                         bg='gray91', activebackground='gray78' , command = lambda : Speed_calculations.entering_numbers('.'))
    ...
    Result_button = Button(root , text = 'Calculate' , font=('arial', 20, 'bold') ,  width=9, height=1 , bd=4 ,
                           bg="SteelBlue1" , activebackground = 'SteelBlue3' , command = Speed_calculations.calculate)
    ...
    Clear_last_button = Button(root, text='⌫', width=9, height=1, font=('arial', 20, 'bold'), bd=4,
                               bg="gray91", activebackground='gray78' , command = Speed_calculations.remove_last)
    ...
    AllClear_button = Button(root, text=chr(67), width=9, height=1, font=('arial', 20, 'bold'), bd=4,
                             bg="gray91", activebackground='gray78' , command = Speed_calculations.clear_entry)
